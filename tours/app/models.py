...
class User(UserMixin, db.Model):
    ...
    username: so.MappedColumn[str] = so.mapped_column(sa.String(60), unique=True, index=True)
    ...
    user_tour: so.WriteOnlyMapped['Tour'] = so.relationship('Tour', secondary=user_tour, back_populates='users')
...
class Tour(db.Model):
    ...
    users: so.WriteOnlyMapped['Tour'] = so.relationship('User', secondary=user_tour, back_populates='user_tour')
